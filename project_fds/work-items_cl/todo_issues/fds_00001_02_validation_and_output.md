...
MVP 범위: 인가 통제형 Keycloak 기반 프록시 게이트웨이 (화이트리스트 기반)
...
### ~~관리 API (`/api/admin/`)~~ — 제거 예정 (DB 직접 CRUD로 대체)
- [x] ~~`X-ADMIN-KEY` 헤더 누락 시 401 응답~~ → Admin API 제거
- [x] ~~`X-ADMIN-KEY` 헤더 불일치 시 403 응답~~ → Admin API 제거
...
### 프록시 (`/{sub_prefix}/{path:path}`) — 루트 레벨, Keycloak JWT 인증
- [x] 등록된 서비스로 프록시 요청 정상 전달 (Conf→401, Jira→403 = upstream 도달)
- [x] 외부 서비스 토큰으로 인가 헤더 주입 (bearer)
...
- [x] ~~Admin API: X-ADMIN-KEY 없으면 접근 불가~~ → Admin API 제거, Keycloak JWT 인증으로 대체
...
| `app/deps.py` | 의존성 (DB 세션, Keycloak JWT 검증) |
...
| `app/routers/proxy.py` | 프록시 라우터 (Keycloak JWT, 화이트리스트) |
| `app/auth/keycloak.py` | Keycloak JWT 검증 (JWKS, 신규) |
